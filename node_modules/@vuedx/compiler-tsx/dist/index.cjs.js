    ...
    if (templateAstTypes.isSimpleExpressionNode(dir.arg)) {
        if (dir.arg.isStatic) {
            if (dir.arg.content.includes(':')) {
                code.push('{...({"', compilerCore.createSimpleExpression(getEventName(dir.arg.content), false, dir.arg.loc), '":', ...exp, '})}');
            }
            else {
                code.push(compilerCore.createSimpleExpression(getEventName(dir.arg.content), false, dir.arg.loc), '={', ...exp, '}');
            }
...
function getEventName(dir) {
    if (dir.includes(':')) {
        ...
        return shared.camelCase('on-' + parts[0]) + ':' + parts.slice(1).join(':');
    }
    return shared.camelCase('on-' + dir);
}
    ...
    return compilerCore.createStructuralDirectiveTransform(/^(if|else-if|else)$/, (node, dir, context) => {
        ...
        return compilerCore.processIf(node, dir, context, (ifNode, branch, isRoot) => {
            return () => {
                ...
                ifNode.codegenNode = compilerCore.createCompoundExpression(expressions);
            };
        });
    });
}
